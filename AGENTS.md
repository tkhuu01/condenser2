...
PostgreSQL row transfer always uses `copy_rows` in `psql_database_helper.py`, which pipes block-level `COPY ... TO STDOUT` straight into `COPY ... FROM STDIN` via a staging table when `ON CONFLICT` deduplication is needed. JSON values pass through as raw COPY text. Generated columns are excluded via the COPY column list; identity values are preserved via `OVERRIDING SYSTEM VALUE` on the staging insert. MySQL continues to use its database-specific bulk insert helper.
...
- Incremental inserts upsert using the resolved incremental identity from `_conflict_clause`: `ON CONFLICT (<identity>) DO UPDATE`, guarded by `IS DISTINCT FROM` so unchanged rows write nothing. The identity may be the primary key or an eligible unique key. Re-read rows therefore refresh in place (history `enddate`s, soft-delete flags). The COPY staging insert dedups on the resolved identity (`DISTINCT ON`) first, since one upsert statement cannot affect a row twice.
- Within each incremental copy, staged COPY rows refresh existing rows before new-row inserts; the sequential staging insert orders them via `ORDER BY EXISTS(...) DESC`. This lets deactivate-and-replace patterns load under a live partial unique index (e.g. one active history row per entity): the stale row is switched off before its replacement arrives. It cannot cover a stale row whose source counterpart was hard-deleted (nothing refreshes it); that still errors loudly and requires a recreate run. Downstream tables with secondary uniqueness accumulate all candidate-key groups and ID batches in one session-local staging table, then apply one refresh phase before the insert phase. In incremental runs, ctid-parallel splitting of tables with a secondary unique index or exclusion constraint (detected at `prep_incremental`, see `has_secondary_unique`) likewise runs in two phases: every worker stages its page range and applies refreshes, all workers meet at a barrier, then inserts run (`stage_rows`/`apply_staged`) — same guarantee as a sequential copy at full worker count. Identity-only tables keep the single-pass split. The two-phase path needs an identity-tracked delta; otherwise those tables fall back to a sequential copy.
...
- Fixture `subsetter_dbs` is parameterized: every test runs twice (`[unnest]` and `[temp_tables]`); both use PostgreSQL COPY for row transfer
- Fixtures `rerun_dbs`, `incremental_dbs`, and `grow_dbs` cover re-runs into an existing destination (idempotency, top-up, and grow semantics) across both ID lookup variants; `incremental_composite_dbs` covers the composite-PK downstream delta join
- Each variant gets its own databases (e.g. `condenser_test_source` and `condenser_test_source_temp`)
