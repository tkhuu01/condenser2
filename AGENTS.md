...
- Every insert records PKs into a per-table delta table in the reserved `_condenser` schema (unlogged, dropped only after a successful run and FK restoration) via `WITH ins AS (INSERT ... RETURNING <pk>, (xmax = 0) AS _inserted) INSERT INTO <delta> ...`. The `_inserted` flag distinguishes genuine inserts from upsert refreshes. Failed runs retain the journal so the next incremental run with the same configuration can resume; a destination advisory lock rejects concurrent incremental runs.
