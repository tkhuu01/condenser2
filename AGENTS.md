...
- Within each incremental copy, staged COPY rows refresh existing rows before new-row inserts; the sequential staging insert orders them via `ORDER BY EXISTS(...) DESC`. This lets deactivate-and-replace patterns load under a live partial unique index (e.g. one active history row per entity): the stale row is switched off before its replacement arrives. It cannot cover a stale row whose source counterpart was hard-deleted (nothing refreshes it); that still errors loudly and requires a recreate run. In incremental runs, ctid-parallel splitting of tables with a secondary unique index or exclusion constraint (detected at `prep_incremental`, see `has_secondary_unique`) runs in two phases: every worker stages its page range and applies refreshes, all workers meet at a barrier, then inserts run (`stage_rows`/`apply_staged`) — same guarantee as a sequential copy at full worker count. Identity-only tables keep the single-pass split. The two-phase path needs an identity-tracked delta; otherwise those tables fall back to a sequential copy.
